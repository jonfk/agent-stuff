...
def assert_can_write_explicit_outputs(md_path: Path, vtt_path: Path, want_subtitle: bool, overwrite: bool) -> None:
    if overwrite:
        return

    existing = [str(md_path)] if md_path.exists() else []
    if want_subtitle and vtt_path.exists():
        existing.append(str(vtt_path))
    if existing:
        joined = ", ".join(existing)
        raise UserFacingError(f"output file already exists: {joined} (use --overwrite)")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Output:\n"
            "  By default, writes: ./<kebab-title>--<video-id>.md\n"
            "  With -o/--output, writes Markdown to the given file path.\n"
            "  With --subtitle, also writes a .vtt file beside the Markdown output.\n\n"
            "Notes:\n"
    ...
    parser.add_argument("-o", "--output", help="Markdown output file (default: generated filename in current directory)")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.output:
        ...
        vtt_out = md_out.with_suffix(".vtt")
        md_out.parent.mkdir(parents=True, exist_ok=True)
        if args.subtitle:
            vtt_out.parent.mkdir(parents=True, exist_ok=True)
        assert_can_write_explicit_outputs(md_out, vtt_out, args.subtitle, args.overwrite)
    else:
        out_dir = Path(".")
